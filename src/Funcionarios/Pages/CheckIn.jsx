...
const CheckInFuncionario = () => {
  ...
  const [ idCurrentReservation, setIdCurrentReservation ] = useState(0)
  const [ idcurrentUsuario, setIdCurrentUsuario ] = useState(0)
  
  const [ idCurrentDpto, setIdCurrentDpto ] = useState(0)
  ...
  const evaluateStatusBtn = (estadoRespuesta,idReserva,idDepto,usuarioId) => {
    const estadoValue = estadoRespuesta === 'Pendiente' ? 2 : 1;
    switch (estadoValue) {
      case 2:
        return (
        ...
        case 1:
          return (
              <div className="font-extrabold flex flex-col items-center gap-2 ">
                ...
                <button onClick={(e)=>{handleOpenModalCheckOut(e,idReserva,idDepto,usuarioId)}} className="rounded-lg  relative w-40 h-10 overflow-x-hidden cursor-pointer flex items-center border border-red-500 bg-red-300 group active:bg-red-500 active:border-red-500" href="{{ route('process.create') }}">
                  <span className="text-white font-bold ml-8 transform group-hover:translate-x-20 transition-all duration-300">Salida</span>
  ...
  const handleOpenModalCheckOut = (e,idReserva,idDepto,usuarioId) => {
    e.preventDefault();
    setIdCurrentReservation(parseInt(idReserva))
    setIdCurrentDpto(parseInt(idDepto))
    setIdCurrentUsuario(parseInt(usuarioId))
  ...
  return (
    ...
    <div className="bg-gray-100 p-1">
                ...
                .map((checkInItem) => (
                  <tr key={checkInItem.RESERVA_ID} className="hover:bg-gray-100 italic border-b-4">
                    ...
                    <td className="px-4 py-4 text-sm whitespace-nowrap text-center">
                      {evaluateStatusBtn(checkInItem.ESTADO,checkInItem.RESERVA_ID,checkInItem.DEPARTAMENTO_ID,checkInItem.USUARIO_ID)}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <ModalRegistroSalida showModal={showModalOut} onClose={closeModalOut} idReserva={idCurrentReservation} idDepto={idCurrentDpto} idUsuario={idcurrentUsuario}/>
  </section>
    </>
  );
};
