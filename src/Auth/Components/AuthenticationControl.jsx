...
import { PublicRoutes, Role_user } from '../../Router';

const AuthenticationControl = ({ user }) => {
  const { logout } = useSession();
  const navigate = useNavigate ();
  const isClientLoggedIn = user && user.role === Role_user.CLIENTE && !!user.access_token;

  useEffect(() => {
    if (user && user.role !== Role_user.CLIENTE) {
      logout();
      navigate(PublicRoutes.LOGIN);
    }
  }, [user, logout, navigate]);

  if (isClientLoggedIn && user.role === Role_user.CLIENTE) {
    return <UserMenu />;
  } else {
    return (
