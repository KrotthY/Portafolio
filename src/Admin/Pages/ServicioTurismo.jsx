...
const ServicioTurismo = () => {
  ...
  const evaluateStatusBtn = (idTour) => {
    return (
      <div className="font-extrabold flex items-center gap-x-6">
        <Tooltip content="Editar Tour" placement="top">
        <button onClick={(e)=>{handleOpenModalEdit(e,idTour)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-blue-500 bg-blue-300 group  active:bg-blue-500 active:border-blue-500" href="{{ route('process.create') }}">
        ...
        <Tooltip content="Borrar tour" placement="top">
        <button onClick={(e)=>{handleTourDelete(e,idTour)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-red-500 bg-red-300 group active:bg-red-500 active:border-red-500" href="{{ route('process.create') }}">
  ...
  const evaluateStatusDepto = (estadoTour) => {
    ...
    switch (tipoEstado) {
      case 1:
        ...
        value="Activo"
        icon={
          <span className="mx-auto  block h-3 w-3 rounded-full bg-green-900 content-['']" />
        }
      />
        );
      ...
      case 2:
        return (
          ...
          value="Inactivo"
          icon={
            <span className="mx-auto block h-3 w-3  bg-red-900 content-['']" />
          }
        />
  ...
  const handleTourDelete = async (e, tourId) => {
    e.preventDefault();
    try {
      ...
      const confirmResult = await Swal.fire({
        ...
        confirmButtonText: 'Si, borrar tour',
  ...
  return (
    <>
      ...
      <Typography variant="h3">
        Administración de Servicios Turismo
      </Typography>
      </div>        
      <div className="flex flex-col justify-start gap-2">
          <Typography variant="h5" color="blue-gray">
            Lista de Servicios Turismo
          </Typography>
          <Typography color="gray" className=" font-normal">
          En esta Seccion podras gestionar los servicios de turismo que se ofrecen segun su localidad.
          </Typography>
      </div>
        ...
        <div className="bg-white  relative shadow-2xl overflow-hidden">
          <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            ...
            <div>
              ...
              > Crear servicio de turismo </button>
            </div>
          </div>
