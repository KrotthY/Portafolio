...
import { ModalCreateTraslado } from "../Components";
import { desahabilitarTraslado } from "../Api";

const URL_API_GET_VEHICULOS = 'https://fastapi-gv342xsbja-tl.a.run.app/vehiculos';
...
const ServiciosTransportes = () => {
  ...
  const cargarVehiculos = () => {
    ...
    fetch(URL_API_GET_VEHICULOS,requestOptions)
  ...
  const evaluateStatusBtn = (idVehiculo,estado) => {
    ...
    return (
      ...
      <div className="font-extrabold flex items-center gap-x-6">
        ...
        <Tooltip content="Desahabilitar vehiculo" placement="top">
        <button onClick={(e)=>{handleTourDelete(e,idVehiculo,changeStatus)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-red-500 bg-red-300 group active:bg-red-500 active:border-red-500" >
        ...
        <Tooltip content="Habilitar vehiculo" placement="top">
        <button onClick={(e)=>{handleTourDelete(e,idVehiculo,changeStatus)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-blue-500 bg-blue-300 group active:bg-blue-500 active:border-blue-500" >
  ...
  const filteredVehiculo = vehiculo.filter((vehiculoItem) => {
    return Object.values(vehiculoItem).some(value => value.toString().toLowerCase().includes(searchTerm.toLowerCase()));
  })

  const totalItems = filteredVehiculo.length;
  ...
  const handleTourDelete = async (e, idVehiculo,estado) => {
    e.preventDefault();
    try {
      const message = estado === "N" ? "¿Deseas deshabilitar el vehiculo?" : "¿Deseas habilitar el vehiculo?";
      const confirmResult = await Swal.fire({
        ...
        confirmButtonText: 'Si, actualizar vehiculo',
      ...
      if (confirmResult.isConfirmed) {
        ...
        await desahabilitarTraslado(borrarTour);
        Swal.fire({
          icon: 'success',
  ...
  return (
    <>
      ...
      <div className="flex flex-col justify-start gap-2">
          <Typography variant="h5" color="blue-gray">
            Lista de Transalados
          </Typography>
          <Typography color="gray" className=" font-normal">
          En esta seccion podras gestionar los servicios de traslados.
          </Typography>
      </div>
            ...
            <table className="w-full text-sm text-left text-gray-500  ">
              <thead className="text-md text-gray-700 bg-blue-100  text-center">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3 w-auto">Patente</th> 
                  <th scope="col" className="px-4 py-3 w-auto">Plazas</th> 
              ...
              <tbody>
                ...
                .map((vehiculoItem) => (
                  <tr key={vehiculoItem.VEHICULO_ID} className="hover:bg-gray-100 italic text-center">
                    ...
                    <td className="px-4 py-4 text-sm text-gray-500 whitespace-nowrap ">{vehiculoItem.PATENTE}</td>
                    <td className="px-4 py-4 text-sm text-gray-500 whitespace-nowrap ">{ vehiculoItem.PLAZAS }</td>
                    <td className="px-4 py-4 text-sm flex items-center justify-center">
                      {evaluateStatusBtn( vehiculoItem.VEHICULO_ID,vehiculoItem.ACTIVO)}
                    </td>
                  </tr>
                ))}
              </tbody>
    ...
    <ModalCreateTraslado onClose={closeModalCreate} showModal={showModalCreate} />
...
export default ServiciosTransportes;
