...
import { disabledServicios } from "../Api/Servicios/desahabilitarServicios";
import { ModalCreateServicios, ModalEditServicios } from "../Components";

const URL_API_GET_SERVICIOS = 'https://fastapi-gv342xsbja-tl.a.run.app/servicios';
...
const Servicios = () => {
  ...
  const cargarServicios =  () => {
    ...
    fetch(URL_API_GET_SERVICIOS,requestOptions)
  ...
  const evaluateStatusBtn = (idServicio, estado) => {

    const estadoValue = estado === "S" ? 1 : 2;
    return (
      ...
      <div className="font-extrabold flex items-center gap-x-6">
        ...
        <Tooltip content="Desahabilitar servicio" placement="top">
        <button onClick={(e)=>{handleServiceDisabled(e,idServicio,estado)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-red-500 bg-red-300 group active:bg-red-500 active:border-red-500" href="{{ route('process.create') }}">
        ...
        <Tooltip content="Habilitar servicio" placement="top">
        <button onClick={(e)=>{handleServiceDisabled(e,idServicio,estado)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-blue-500 bg-blue-300 group active:bg-blue-500 active:border-blue-500" href="{{ route('process.create') }}">
          <span className="absolute right-0 h-full w-11 rounded-lg bg-blue-300 hover:bg-blue-500 flex items-center justify-center transform group-hover:translate-x-0 group-hover:w-full transition-all duration-300">
  ...
  const evaluateStatusDepto = (estadoDpto) => {
    const estadoValue = estadoDpto === "S" ? 1 : 2;
    switch (estadoValue) {
      case 1:
        return (
  ...
  const handleServiceDisabled = async (e, servicioId, estado) => {
    e.preventDefault();
    try {
      const message = estado === "N" ? "¿Deseas habilitar el servicio?" : "¿Deseas deshabilitar el servicio?";
      const change = estado === "N" ? "S" : "N";
      ...
      const confirmResult = await Swal.fire({
        ...
        confirmButtonText: 'Si, actualizar servicio',
      ...
      if (confirmResult.isConfirmed) {
        ...
        await disabledServicios(borrarServicios);
        Swal.fire({
          icon: 'success',
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-gray-500  ">
              ...
              <tbody>
                ...
                .map((serviciosItem) => (
                  <tr key={serviciosItem.SERVICIO_ID} className="hover:bg-gray-100 italic text-center  border-b-4">
                    <td className="px-4 py-4 text-sm text-gray-500 whitespace-nowrap">{serviciosItem.NOMBRE}</td>
                    <td className="px-4 py-4 ">{evaluateStatusDepto(serviciosItem.ACTIVO)}</td>

                    <td className="px-4 py-4 text-sm flex items-center justify-center">
                      {evaluateStatusBtn( serviciosItem.SERVICIO_ID,serviciosItem.ACTIVO)}
                    </td>
                  </tr>
                ))}
              </tbody>
