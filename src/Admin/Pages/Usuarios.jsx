...
const Usuarios = () => {
  ...
  const evaluateStatusBtn = (idServicio, estado) => {
    ...
    return (
      ...
      <div className="font-extrabold flex items-center gap-x-6">
        ...
        <button onClick={(e)=>{handleServiceDelete(e,idServicio,estado)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-red-500 bg-red-300 group active:bg-red-500 active:border-red-500" href="{{ route('process.create') }}">
        ...
        <button onClick={(e)=>{handleServiceDelete(e,idServicio,estado)}} className="rounded-lg  relative w-12 h-10 overflow-x-hidden cursor-pointer flex items-center border border-blue-500 bg-blue-300 group active:bg-blue-500 active:border-blue-500" href="{{ route('process.create') }}">
          <span className="absolute right-0 h-full w-11 rounded-lg bg-blue-300 hover:bg-blue-500 flex items-center justify-center transform group-hover:translate-x-0 group-hover:w-full transition-all duration-300">
  ...
  const handleServiceDelete = async (e, servicioId, estado) => {
    e.preventDefault();
    try {
                ...
                .map((serviciosItem) => (
                  <tr key={serviciosItem.SERVICIO_ID} className="hover:bg-gray-100 italic text-center">
                    ...
                    <td className="px-4 py-4 text-sm flex items-center justify-center">
                      {evaluateStatusBtn( serviciosItem.SERVICIO_ID,"S")}
                    </td>
                  </tr>
                ))}
              </tbody>
