...
const ModalEditTour = ({onClose,showModal,tourId}) => {
  ...
  useEffect(() => {
    if(tourIdSelected){
      ...
      setValue('duracion', tourIdSelected?.DURACION);
      setValue('descripcion', tourIdSelected?.DESCRIPCION);
      setValue('precio', tourIdSelected?.VALOR_MINIMO);
      setValue('capacidad', tourIdSelected?.CAPACIDAD_PARTICIPANTES);
      setValue('active', tourIdSelected?.ACTIVO === 'S'? true : false);
  ...
  return (
    ...
    >
      <DialogHeader className="border-b-2 border-gray-300 flex justify-between items-start p-5">
        <span className="text-2xl tracking-tight font-extrabold text-gray-900">Editar servicio turismo </span>
